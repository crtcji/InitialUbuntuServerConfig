...
# Configuring UFW, updating/upgrading, installing necessary cli apps
yes | ufw limit 22/tcp && yes | ufw limit 7539/tcp;
ufw allow 1194/udp && ufw enable && echo "IPV6=no" >> /etc/ufw/ufw.conf && ufw reload && apt -y update && apt -y upgrade && apt -y dist-upgrade && apt -y install mc screen git whois htop arp-scan curl iptraf sysbench unattended-upgrades glances ntp ntpdate rcconf rig shellcheck sysv-rc-conf tmux;
...
#
# sctn_echo ANTIVIRUS "(Clam-AV)" > $dn >> $rlog;
# bckup $clmcnf;
# mkdir -p $rprtfldr;
#
# # Enabling "SafeBrowsing true" mode
# enbl_echo SafeBrowsing > $dn >> $rlog;
...
# echo -e "Creating a \e[1m\e[34mcronjob\e[0m for the ClamAV ..." > $dn >> $rlog;
